...
namespace Mori.SDK.GoogleImporter
{
    public class ConfigImportsMenu
    {
        ...
        private const string SETTINGS_FILE_NAME = "InventorySettings";
        ...
        private static InventorySettings LoadSettings()
        {
            var jsonLoaded = PlayerPrefs.GetString(SETTINGS_FILE_NAME);
            var inventorySettings = !string.IsNullOrEmpty(jsonLoaded)
                ? JsonUtility.FromJson<InventorySettings>(jsonLoaded)
